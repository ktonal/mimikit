...
from . import GatedConv1d, LearnablePad1d
...
def accum(x, y, shift=1):
    ...
    rest = y[:, :, compl_y]
    return torch.cat((aligned, rest)[::side], dim=-1).contiguous()
...
def concat(x, y, shift=1):
    ...
    return torch.cat((x[:, :, compl_x], y)[::side], dim=-1).contiguous()
...
class DilatedConv1d(nn.Module):
    ...
    def __init__(self,
        ...
        self.gate = GatedConv1d(self.input_dim, self.layer_dim, self.kernel_size,
                                local_cond_dim=local_cond_dim, global_cond_dim=global_cond_dim,
                                **convs_kwargs)
    ...
    def forward(self, x, skip=None, local_cond=None, global_cond=None):
        input = self.pad(x)

        y = self.gate(input, local_cond, global_cond)
