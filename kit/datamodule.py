...
from mmk.kit.ds_wrappers import MultiFeatureSet, IterableDatasetWrapper, is_map_style_dataset
from mmk.data.metadata import Metadata


def is_wrapper_ds(dataset):
    return issubclass(type(dataset), MultiFeatureSet) or issubclass(type(dataset), IterableDatasetWrapper)


def zip_stack(batch_lists):
    """
    @batch_lists: ((feat_1_1, feat_2_1), (feat_1_2, feat_2_2), ...)
...
class MMKDataModule(pl.LightningDataModule):
    ...
    def __init__(self,
                 ...
                 ds_wrapper: [MultiFeatureSet, IterableDatasetWrapper] = None,
                 train_val_split=1.,
                 **loader_kwargs,
                 ):
    ...
    def _prepare_feature(self):
        ...
        if ds_wrapper is not None:
            ...
            if is_map_style_dataset(dataset[0]):
                dataset = MultiFeatureSet()(*dataset)
            else:
    ...
    def _wrap_dataset(feat, wrapper):
        if issubclass(wrapper, MultiFeatureSet):
